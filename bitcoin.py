...
class VarIntField(FieldLenField):
    ...
    def i2m(self, pkt, x):
        if x is None:
            if self.length_of is not None or self.count_of is not None:
                counter = self.length_of or self.count_of
                fld, fval = pkt.getfield_and_val(counter)
    ...
    def addfield(self, pkt, s, val):
        ...
        if val < 0xfd:
            ...
            f_s = b"\xfd" + struct.pack(">H", val)
        elif val <= 0xffffffff:
            f_s = b"\xfe" + struct.pack(">I", val)
        else:
            f_s = b"\xff" + struct.pack(">L", val)
...
class BitcoinHdr(Packet):
    ...
    def cmdstr(self):
        ...
        return val.decode().replace("\x00", "")

    def guess_payload_class(self, payload):
        fld, val = self.getfield_and_val("cmd")
        cmd = val.decode().replace("\x00", "")
...
class BitcoinGetdata(BitcoinMessage):
    ...
    fields_desc = [
        VarIntField("count",None, length_of="inventory"),
        PacketListField("inventory", [], InventoryPktField, length_from=lambda pkt: pkt.count),
    ]
